...
# The `render_action_item_tracker` function itself is hard to test directly due
# to Streamlit calls. A better approach is to refactor the logic into a pure
# function that can be tested, similar to the traceability matrix example.

def aggregate_and_enrich_actions(ssm: SessionStateManager) -> pd.DataFrame:
    '''Refactored logic for testing purposes.'''
    all_actions = []
    # (Copy the aggregation logic from the main function here)
    reviews = ssm.get_data("design_reviews", "reviews")
    for i, review in enumerate(reviews):
        for action in review.get("action_items", []):
            action_copy = action.copy()
...
@pytest.fixture
def mock_ssm_for_actions():
    '''Mocks SessionStateManager with action items from different sources.'''
    ssm = MagicMock()
    today = date.today()
        ...
        ("design_reviews", "reviews"): [
            {
                'date': '2023-01-01',
                'action_items': [
                    {'id': 'AI-01', 'owner': 'Alice', 'status': 'Open', 'due_date': str(today + timedelta(days=10))},
                    {'id': 'AI-02', 'owner': 'Bob', 'status': 'In Progress', 'due_date': str(today - timedelta(days=1))}, # Should become Overdue
                ]
            }
        ],
        ("design_changes", "changes"): [ # This source is not yet used in the provided code but shows extensibility
            # Empty for now to test robustness
        ],
    }.get((primary_key, secondary_key), [])
    return ssm


def test_action_aggregation(mock_ssm_for_actions):
    '''Tests that actions are correctly aggregated from all sources.'''
    df = aggregate_and_enrich_actions(mock_ssm_for_actions)
    assert len(df) == 2
    assert 'AI-01' in df['id'].values
    assert 'AI-02' in df['id'].values

def test_overdue_status_enrichment(mock_ssm_for_actions):
    '''Tests that the 'Overdue' status is correctly applied.'''
    df = aggregate_and_enrich_actions(mock_ssm_for_actions)
    
    # AI-01 should remain 'Open'
    status_ai01 = df[df['id'] == 'AI-01']['status'].iloc[0]
    assert status_ai01 == 'Open'
    
    # AI-02 was 'In Progress' but its due date is in the past, so it should be 'Overdue'
    status_ai02 = df[df['id'] == 'AI-02']['status'].iloc[0]
    assert status_ai02 == 'Overdue'

def test_empty_action_items():
    '''Tests graceful handling of no action items.'''
    ssm = MagicMock()
    ssm.get_data.return_value = []
    df = aggregate_and_enrich_actions(ssm)
    assert df.empty
"""
