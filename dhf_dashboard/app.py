...
# ==============================================================================
"""
# To run tests for this application, a good strategy is to refactor key data
# processing logic into pure, testable functions that do not depend on Streamlit's
# state or UI rendering.

def test_gantt_chart_data_processing():
    '''
    Tests the data pre-processing logic for the Gantt chart.
    Note: This requires mocking the SessionStateManager and find_critical_path.
    '''
    from unittest.mock import MagicMock
    import dhf_dashboard.utils.critical_path_utils as cp_utils

    # 1. Arrange
    mock_ssm = MagicMock(spec=SessionStateManager)
    mock_tasks = [
        {"id": "T1", "name": "Task 1", "start_date": "2023-01-01", "end_date": "2023-01-10", "status": "Completed", "completion_pct": 100},
        {"id": "T2", "name": "Task 2", "start_date": "2023-01-11", "end_date": "2023-01-20", "status": "In Progress", "completion_pct": 50}
    ]
    mock_ssm.get_data.return_value = mock_tasks

    cp_utils.find_critical_path = MagicMock(return_value=["T1", "T2"])

    # In a real test suite, you'd refactor the processing logic from main()
    # into a testable function like `prepare_gantt_data(tasks: list) -> pd.DataFrame`.
    # For this example, we simulate the logic directly.
    tasks_df_processed = pd.DataFrame(mock_tasks)
    tasks_df_processed['start_date'] = pd.to_datetime(tasks_df_processed['start_date'])
    tasks_df_processed['end_date'] = pd.to_datetime(tasks_df_processed['end_date'])
    critical_path_ids = cp_utils.find_critical_path(tasks_df_processed)
    tasks_df_processed['is_critical'] = tasks_df_processed['id'].isin(critical_path_ids)
    tasks_df_processed['line_color'] = np.where(tasks_df_processed['is_critical'], 'red', '#FFFFFF')

    # 3. Assert
    assert not tasks_df_processed.empty
    assert 'line_color' in tasks_df_processed.columns
    assert tasks_df_processed[tasks_df_processed['id'] == 'T1']['line_color'].iloc[0] == 'red'
    assert tasks_df_processed[tasks_df_processed['id'] == 'T2']['line_color'].iloc[0] == 'red'
"""
